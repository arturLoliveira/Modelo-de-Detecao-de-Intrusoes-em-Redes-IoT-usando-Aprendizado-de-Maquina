...
print("DETECÇÃO DE INTRUSÃO - UNSW-NB15 (SVM Linear - Exportação)")
...
# 3.2 Imputação e Normalização (Crítico para SVM)
...
report_path = os.path.join(OUTPUT_DIR, 'relatorio_unsw_svm.txt')
with open(report_path, 'w') as f:
    f.write("RELATORIO DE PERFORMANCE - UNSW-NB15 (SVM)\n")
    f.write("="*60 + "\n\n")
...
# 7.1 Feature Importance (Pesos do SVM)
base_model = model.calibrated_classifiers_[0].estimator
importances = np.abs(base_model.coef_[0])
indices = np.argsort(importances)[::-1][:20]

plt.figure(figsize=(12, 6))
plt.title("Top 20 Features - UNSW-NB15 (SVM Weights)", fontsize=14)
plt.bar(range(20), importances[indices], color='magenta', edgecolor='purple')
plt.xticks(range(20), [X_train.columns[i] for i in indices], rotation=45, ha='right')
plt.tight_layout()
...
# 7.2 Matriz de Confusão (Val e Teste)
...
ConfusionMatrixDisplay(cm_test, display_labels=['Normal', 'Attack']).plot(ax=axes[1], cmap='Purples')
...
ax.set_title('SVM: Comparação Treino vs Teste')
ax.set_xticks(x)
